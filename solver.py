...
from model import weights_init,LANet,Discriminate
...
class Solver(object):
    def __init__(self, train_loader, val_loader, test_loader, config):
        ...
        self.build_model()
        if self.config.pre_trained: self.generator.load_state_dict(torch.load(self.config.pre_trained))
        if config.mode == 'train':
            ...
            self.test_output02 = open("%s/pre_and_recall.txt" % config.test_fold,'w')
    ...
    def test(self, num):
        ...
        for i, data_batch in enumerate(self.test_loader):
            with torch.no_grad():
                ...
                #prob = (prob_pred.cpu().data[0][0].numpy() * 255).astype(np.uint8)
                #p_img = Image.fromarray(prob, mode='L').resize(shape)
                #name = str(i+1)
                #p_img.save('predict/'+name+'.png')

            mae = self.eval_mae(prob_pred, labels)
            f = open('mae.txt','w+')
            f.writelines(str(mae)+'\n')
            f.close()
        ...
        for i in range(len(avg_recall)):
            print('%.4f  %.4f'%(avg_prec[i],avg_recall[i]),file=self.test_output02)
